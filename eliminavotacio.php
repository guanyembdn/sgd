...
if(isset($_POST["id"]) && !empty($_GET["id"])){
		$sth = $con->prepare("DELETE FROM votacions WHERE id=?");
		$sth->execute(array(intval($_POST["id"])));
		?>
		
		<p>La votació ha sigut eliminada.<br>
		<ul>
		<?php if(!isadmin()) { ?> <li><a href="lesmevesvotacions">Anar a les meves votacions</a></li> <?php } ?>
...
<h1>Eliminar aquesta votació</h1>
...
<input type="submit" value="Eliminar votació">
</form>
